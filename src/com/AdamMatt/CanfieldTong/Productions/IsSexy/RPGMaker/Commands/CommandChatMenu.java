...
public class CommandChatMenu {
    ...
    public List<String> getChatMenu() {
        ...
        for (int i = 0; i < commandDescription.size(); i++) {
            out.add(ChatColor.DARK_GRAY + commandDescription.get(i));
        }
        if (subCommands != null) {
            out.add("");
            for (int i = 0; i < subCommands.size(); i++) {
                out.add(ChatColor.YELLOW + subCommands.get(i).commandUsage + ChatColor.DARK_GRAY + " - " + subCommands.get(i).commandName + subCommands.get(i).quickDescription);
            }
        }
        out.add("");
        return out;
